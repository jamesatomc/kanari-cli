...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            kanari_token: deps.api.addr_canonicalize(&msg.kanari_token)?,
        },
    )?;
...
pub fn claim(
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: deps.api.addr_humanize(&config.kanari_token)?.to_string(),
            funds: vec![],
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        ...
        kanari_token: deps.api.addr_humanize(&state.kanari_token)?.to_string(),
    };
